    ...
    activate(ctx, r) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        ctx.spawnProjectile({
          ...
          dmg: p.damage * (o.dmg || 0.8), pierce: o.pierce || 0, r: o.size || 6, color: o.color,
        });
      }
    },
  };
    ...
    activate(ctx, r) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        ctx.spawnProjectile({
          ...
          dmg: p.damage * (o.dmg || 0.7), pierce: o.pierce || 0, r: o.size || 6, color: o.color,
        });
      }
    },
  };
...
const ABILITIES = [
  ...
  volleySkill({ id: 'kn_spears', name: 'Spear Throw', classId: 'knight', maxRank: 5, icon: '🗡', flavor: 'Hurl spears', cd: 1.6, cdStep: 0.1, count: 1, countStep: 1, dmg: 1.0, pierce: 2, size: 7, color: '#cdd6e6' }),
  ...
  // Archer — ranged
  volleySkill({ id: 'ar_multi', name: 'Multishot', classId: 'archer', maxRank: 6, icon: '➶', flavor: 'Fan of arrows', cd: 1.3, cdStep: 0.08, count: 3, countStep: 1, dmg: 0.7, pierce: 1, size: 5, color: '#cdeccd' }),
  ...
  radialSkill({ id: 'ar_scatter', name: 'Scattershot', classId: 'archer', maxRank: 5, icon: '✺', flavor: 'Arrows burst outward', cd: 2.4, cdStep: 0.15, count: 6, countStep: 2, dmg: 0.6, pierce: 1, size: 5, color: '#cdeccd' }),
  volleySkill({ id: 'ar_power', name: 'Power Shot', classId: 'archer', maxRank: 5, icon: '➳', flavor: 'Heavy piercing arrow', cd: 1.8, cdStep: 0.12, count: 1, countStep: 0, dmg: 2.4, pierce: 6, size: 9, speed: 600, color: '#a9e6b4' }),
  ...
  // Rogue — fast & tricky
  volleySkill({ id: 'rg_fan', name: 'Fan of Knives', classId: 'rogue', maxRank: 6, icon: '🔪', flavor: 'Spray of knives', cd: 1.1, cdStep: 0.07, count: 3, countStep: 1, dmg: 0.6, size: 5, spread: 0.9, color: '#f4dd8a' }),
  ...
  radialSkill({ id: 'rg_burst', name: 'Knife Storm', classId: 'rogue', maxRank: 5, icon: '✺', flavor: 'Knives in all directions', cd: 2.2, cdStep: 0.14, count: 7, countStep: 2, dmg: 0.55, size: 5, color: '#f4dd8a' }),
  ...
  volleySkill({ id: 'cl_bolts', name: 'Holy Bolts', classId: 'cleric', maxRank: 5, icon: '➹', flavor: 'Bolts of light', cd: 1.5, cdStep: 0.1, count: 2, countStep: 1, dmg: 0.9, pierce: 1, size: 6, color: '#fff3c4' }),
  ...
  volleySkill({ id: 'bb_axes', name: 'Axe Throw', classId: 'barbarian', maxRank: 5, icon: '🪃', flavor: 'Hurl heavy axes', cd: 1.6, cdStep: 0.1, count: 1, countStep: 1, dmg: 1.6, pierce: 3, size: 9, speed: 380, color: '#d98c5a' }),
